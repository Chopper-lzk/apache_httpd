 ...
 * src running. See the memcached website at http://www.danga.com/memcached/
...
/** Specifies the status of a memcached src */
typedef enum
{
...
/* Custom hash callback function prototype, user for src selection.
...
/**
 * Picks a src based on a hash
 * @param mc The memcache client object to use
 * @param hash Hashed value of a Key
 * @return src that controls specified hash
...
/**
 * src selection compatible with the standard Perl Client.
 */
 ...
 * @param mc The memcache client object to use
 * @param host Hostname of the src
 * @param port Port of the src
 ...
 * @param ns   location of the new src object
...
/**
 * Gets a value from the src, allocating the value out of p
...
/**
 * Gets multiple values from the src, allocating the values out of p
 * @param mc client to use
...
/**
 * Sets a value by key on the src
 * @param mc client to use
 * @param key   null terminated string containing the key
 * @param baton data to store on the src
 * @param data_size   length of data at baton
 * @param timeout time in seconds for the data to live on the src
 * @param flags any flags set by the client for this key
 */
...
/**
 * Adds value by key on the src
 * @param mc client to use
 * @param key   null terminated string containing the key
 * @param baton data to store on the src
 * @param data_size   length of data at baton
 * @param timeout time for the data to live on the src
 * @param flags any flags set by the client for this key
 * @return APR_SUCCESS if the key was added, APR_EEXIST if the key 
 * already exists on the src.
 */
...
/**
 * Replaces value by key on the src
 * @param mc client to use
 * @param key   null terminated string containing the key
 * @param baton data to store on the src
 * @param data_size   length of data at baton
...
/**
 * Query a src's version
 * @param ms    src to query
 * @param p     Pool to allocate answer from
 * @param baton location to store src version string
 */
...
typedef struct
{
    /** Version string of this src */
    const char *version;
    /** Process id of this src process */
    apr_uint32_t pid;
    /** Number of seconds this src has been running */
    apr_uint32_t uptime;
    /** current UNIX time according to the src */
    apr_time_t time;
    ...
    /** Current number of items stored by the src */
    apr_uint32_t curr_items;
    /** Total number of items stored by this src */
    apr_uint32_t total_items;
    /** Current number of bytes used by this src to store items */
    apr_uint64_t bytes;
    /** Number of open connections */
    apr_uint32_t curr_connections;
    /** Total number of connections opened since the src started running */
    apr_uint32_t total_connections;
    /** Number of connection structures allocated by the src */
    ...
    /** Total number of bytes read by this src */
    apr_uint64_t bytes_read;
    /** Total number of bytes sent by this src */
    apr_uint64_t bytes_written;
    /** Number of bytes this src is allowed to use for storage. */
    apr_uint32_t limit_maxbytes;
    /** Number of threads the src is running (if built with threading) */
    apr_uint32_t threads; 
} apr_memcache_stats_t;

/**
 * Query a src for statistics
 * @param ms    src to query
 * @param p     Pool to allocate answer from
